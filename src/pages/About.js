...
const About = () => {
  ...
  const handleDownload = async (blob) => {
    try {
      ...
      link.href = url;
      link.download = `umitsencer-cv.pdf`;
  ...
  return (
    ...
    >
      ...
      <div className="text-center mb-4 sm:mb-6">
        {isClient ? (
          <BlobProvider document={<ResumePDF dataTr={content.tr.resume} dataEn={content.en.resume} />}>
            {({ blob, url, loading, error }) => (
              <button
                onClick={() => blob && handleDownload(blob)}
                className={`nier-button inline-flex items-center gap-2 hover:scale-105 transition-transform ${loading || error ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
      ...
      <div className="nier-panel mb-8 sm:mb-12">
        <h2 className="text-lg sm:text-xl font-nier text-nier-dark mb-4 sm:mb-6">{t('about.certifications.title')}</h2>
        <div className="space-y-4">
          {currentResumeData.certifications.map((cert, index) => (
            <div key={index}>
              <h3 className="font-nier text-nier-dark text-base sm:text-lg">{cert.name}</h3>
