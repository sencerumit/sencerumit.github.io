const translations = {
  en: {
    nav: {
      home: 'Home',
      about: 'About',
      projects: 'Projects',
      articles: 'Articles',
      contact: 'Contact',
      menu: 'Menu'
    },
    home: {
      greeting: 'Hello World_',
      intro: "I'm a",
      role: 'Portfolio Manager',
      description: 'Solution-oriented Economic Researcher, building a career in finance.',
      viewWork: 'About Me',
      getInTouch: 'Get In Touch'
    },
    about: {
      title: 'About Me',
      downloadResume: 'Download Resume',
      professionalOverview: 'Professional Overview',
      contact: 'Contact',
      skills: {
        title: 'Skills',
        digitalSkills: 'Digital Skills'
      },
      experience: {
        title: 'Experience Timeline',
        present: 'Present'
      },
      education: {
        title: 'Education'
      },
      awards: {
        title: 'Awards'
      },
      publications: {
        title: 'Publications'
      },
      interests: {
        title: 'Professional Interests'
      },
      hobbies: {
        title: 'Personal Hobbies'
      },
      volunteerWork: {
        title: 'Volunteer Work'
      }
    },
    projects: {
      title: 'Projects',
      viewProject: 'View Project',
      viewLive: 'View Live',
      viewCode: 'View Code',
      technologies: 'Technologies',
      highlights: 'Highlights'
    },
    articles: {
      title: 'Articles',
      readMore: 'Read More',
      backToArticles: '← Back to Articles',
      loading: 'Loading articles...',
      error: 'Failed to load articles',
      notFound: 'Article not found'
    },
    contactPage: {
      title: 'Contact Me',
      nameLabel: 'Name',
      emailLabel: 'Email',
      subjectLabel: 'Subject',
      messageLabel: 'Message',
      sendButton: 'Send Message',
      connectWithMe: 'Connect With Me',
      githubButton: 'GitHub',
      linkedinButton: 'LinkedIn',
      twitterButton: 'Twitter',
      mediumButton: 'Medium'
    },
    common: {
      loading: 'Loading...',
      error: 'Error',
      notFound: 'Not Found'
    }
  },
  tr: {
    nav: {
      home: 'Ana Sayfa',
      about: 'Hakkımda',
      projects: 'Projeler',
      articles: 'Makaleler',
      contact: 'İletişim',
      menu: 'Menü'
    },
    home: {
      greeting: 'Merhaba Dünya_',
      intro: '  ',
      role: ' ',
      description: 'Çözüm odaklı bir Ekonomik Araştırmacıyım ve Portföy Yöneticisiyim, finans alanında kariyerime devam ediyorum.',
      viewWork: 'Hakkımda',
      getInTouch: 'İletişime Geç'
    },
    about: {
      title: 'Hakkımda',
      downloadResume: 'CV İndir',
      professionalOverview: 'Profesyonel Genel Bakış',
      contact: 'İletişim',
      skills: {
        title: 'Yetenekler',
        digitalSkills: 'Dijital Yetenekler'
      },
      experience: {
        title: 'Deneyim Zaman Çizelgesi',
        present: 'Günümüz'
      },
      education: {
        title: 'Eğitim'
      },
      awards: {
        title: 'Ödüller'
      },
      publications: {
        title: 'Yayınlar'
      },
      interests: {
        title: 'Profesyonel İlgi Alanları'
      },
      hobbies: {
        title: 'Kişisel Hobiler'
      },
      volunteerWork: {
        title: 'Gönüllü Çalışmalar'
      }
    },
    projects: {
      title: 'Projeler',
      viewProject: 'Projeyi Gör',
      viewLive: 'Canlı Gör',
      viewCode: 'Kodu Gör',
      technologies: 'Teknolojiler',
      highlights: 'Öne Çıkanlar'
    },
    articles: {
      title: 'Makaleler',
      readMore: 'Devamını Oku',
      backToArticles: '← Makalelere Dön',
      loading: 'Makaleler yükleniyor...',
      error: 'Makaleler yüklenirken hata oluştu',
      notFound: 'Makale bulunamadı'
    },
    contactPage: {
      title: 'İletişime Geçin',
      nameLabel: 'Adınız',
      emailLabel: 'E-posta Adresiniz',
      subjectLabel: 'Konu',
      messageLabel: 'Mesajınız',
      sendButton: 'Mesaj Gönder',
      connectWithMe: 'Benimle Bağlantı Kurun',
      githubButton: 'GitHub',
      linkedinButton: 'LinkedIn',
      twitterButton: 'Twitter',
      mediumButton: 'Medium'
    },
    common: {
      loading: 'Yükleniyor...',
      error: 'Hata',
      notFound: 'Bulunamadı'
    }
  }
};

export default translations; 